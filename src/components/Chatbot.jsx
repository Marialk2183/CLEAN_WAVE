...
const Chatbot = () => {
  ...
  const CHATBOT_LOGO_URL = "https://cdn-icons-png.flaticon.com/512/4712/4712035.png"; // Example logo
  ...
  return (
    <>
      <Button
        ...
        style={{ width: 68, height: 68, fontSize: "2.2rem", zIndex: 1050, background: '#A8E6CF', color: '#333', boxShadow: '0 6px 24px rgba(0,0,0,0.10)' }}
      ...
      <Modal show={open} onHide={() => setOpen(false)} centered size="sm">
        <Modal.Header closeButton className="bg-primary text-white" style={{ background: '#A8E6CF', color: '#333', borderTopLeftRadius: 12, borderTopRightRadius: 12 }}>
          <Modal.Title>
            <img
              src={CHATBOT_LOGO_URL}
              alt="Chatbot Logo"
              style={{ width: 32, height: 32, borderRadius: '50%', marginRight: 10, verticalAlign: 'middle', background: '#fff' }}
            />
            CleanWave Chatbot
          </Modal.Title>
        </Modal.Header>
        <Modal.Body style={{ background: "#fff", height: 400, overflowY: "auto", borderBottomLeftRadius: 12, borderBottomRightRadius: 12 }}>
          {messages.map((msg, idx) => (
            <div key={idx} className={`mb-2 text-${msg.from === "user" ? "end" : "start"}`}>
              <span className={`badge ${msg.from === "user" ? "bg-primary" : "bg-light text-dark border"}`}>{msg.text}</span>
            </div>
          ))}
        </Modal.Body>
        <Modal.Footer className="bg-white" style={{ borderBottomLeftRadius: 12, borderBottomRightRadius: 12 }}>
          <Form className="w-100" onSubmit={handleSend} autoComplete="off">
            <InputGroup>
              ...
              />
              <Button variant="primary" type="submit" style={{ background: '#A8E6CF', color: '#333', borderRadius: 6, fontWeight: 600, border: 'none' }}>
                Send
              </Button>
            </InputGroup>
          </Form>
